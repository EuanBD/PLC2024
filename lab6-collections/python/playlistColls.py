...
piecesFromThings = [item for playlist in things if isinstance(playlist, list) for item in playlist if isinstance(item, Piece)] #TASK 5.3.(c) -- replace "todo" by a list comprehension
...
shortItemLengths1 = [item.length_secs for item in playlist1 if item.length_secs < 20] # TASK 6.3.(b) -- replace "todo" by a list comprehension
